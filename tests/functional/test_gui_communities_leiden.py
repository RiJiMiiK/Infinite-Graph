...
def test_window_leiden_parameters_are_hidden(qapp) -> None:
...
def test_window_compute_communities_supports_leiden(qapp, sample_result, monkeypatch) -> None:
    window = gui.InfiniteGraphWindow()
    ...
    assert calls == [(sample_result["community_graph"], "leiden", {})]
    assert "Algorithm: Leiden" in window.community_summary_label.text()
